...
from ._yaml import load as yaml_load, odict, naodict

# gets the real root logger end sets it up
...
handler.setFormatter(logging.Formatter('{asctime} {levelname:4s} {name} {message}', style='{'))
...
def logger(name, level='debug'):
    ...
    log.setLevel(levels[level.lower()])
...
def load(source=None):
    ...
    if op.exists(source):
        log.debug("loading normal nao mode")
        with codecs.open(source) as f:
            return yaml_load(f)

    else:
        log.debug("loading from yaml string")
...
class NaoPath(object):
    ...
    def __init__(self, *base):
        b = op.normpath(op.join(*base))
        self.base = b if op.isdir(b) else op.dirname(b)
